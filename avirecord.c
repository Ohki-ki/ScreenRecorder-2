...
VOID GenBitmap(HWND hWnd, BOOL first)
{
	...
	if (first)
	{
		...
		hr = AVISetCompressionMode(hAvi, hBitmap, &acOpt, msInt+ 3);
		
	}
	else
	{
		HRESULT hr = AVIAddFrame(hAvi, hBitmap, msInt + 5);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		hEditMS = CreateWindowEx(WS_EX_CLIENTEDGE, TEXT("EDIT"), TEXT("50"), WS_VISIBLE | WS_CHILD | ES_NUMBER, 10, 30, 310, 20, hWnd, NULL, thisInstance, NULL);
	...
	case WM_COMMAND:
		if ((HWND)lParam == hChooseWnd)
		{
			...
			_stprintf(wszInfo, TEXT("Target: 0x%.8X \'%s\' (%d x %d)"), (INT)hCapTargetWindow, wszClass, rect.right - rect.left, rect.bottom - rect.top);
		...
		else if ((HWND)lParam == hStart)
		{
			...
			GetWindowText(hEditMS,eTxt,msLen+1);
			
			MessageBox(hWnd,eTxt,TEXT(""),MB_OK);
			//_stscanf_s(eTxt, TEXT("%d"), &msInt);
			_stscanf(eTxt, TEXT("%d"), &msInt);
			...
			_stprintf(fName, TEXT("%hu-%hu-%hu %hu-%hu-%hu-%hu.avi"), st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond, st.wMilliseconds);
